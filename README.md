...
已经能用：项目管理、会话与分支（④ 每条回复下「从这里分叉」）、对话流（思考轨 / 工具卡内嵌 diff / 授权卡 / 提问卡 / 技能卡）、模型与思考等级切换、审批三模式与记忆规则、Live Bar 运行态、右栏页签工作区（任务摘要 / 浏览器 / 统计 / 规则 / 事件）+ 下钻（清单 / diff / 文件内容 / 子代理流）+ 浏览器观测抽屉、内嵌浏览器（含前进后退刷新、上传下载、视口联调标记）、电脑控制、**子代理**（委派卡 + 完整过程下钻）、**技能**（项目 / 用户 / 内置三层，`/skill` 与 `/` 候选、设置页管理）、**净值**（基线 → 现在）、统计与规则面板、主题、设置、**MCP 接入**（本地 / 远程，项目级 + 用户级）。
