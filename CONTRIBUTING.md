...
| `basic` | 主界面自检 | 仅日志 |
| `fixture` | 浏览器能力本体 | **25** |
| `dock` | 右栏 ⑦ 全家桶（页签 / 拖拽 / 折叠 / 下钻 / 净值 / 任务摘要三段顺序 / 观测抽屉 / 前后退刷新 / 逐像素对齐 / 视口标记 / `/compact` / `/memory-tidy` / `/skill` / `/` 候选浮层 / 技能归属 / 技能工具卡标记 / 等待授权 / 装不下提示 / 适应宽度）+ ④「从这里分叉」（尖端不给、不署名、送最终回复 id、运行中置灰）+ ④ 工具卡宽度边界 + ④ 长串文本通例（四类容器；右对齐气泡按**文本自身矩形**量——`scrollWidth` 是假绿灯）+ ⑥ Live Bar 居中（带对照） | **255** |
...
| `model` | 模型解析与降级六段 | **61**（7/6/6/6/3/33） |
...
| `ui-logic` | 界面**逻辑漏洞的回归**：失败提示贴在输入卡片（长会话滚到底也看得见）/ 运行中 `/compact` 不吃输入 / 粘贴与拖入同一套反馈 / 附件超上限如实计数 / 搜索命中如实计数 / 浮层 ESC 与点外部关闭 / 起手态不被 error 赶走 / **发出消息后回到底部**（每条都带对照；**不打模型**） | **32** |
| `host` | 宿主能力往返 | 7 |
| `advanced` / `approval` / `reenter` / `crash` | 长会话 / 审批四场景 / 重入 / 崩溃恢复 | 仅日志 |

> 📌 上表的断言条数**仅作量级参考、会随增删漂移**；验收一律以运行输出为准。

> ⚠️ **计费**：只有 `fixture`、`dock`、`skills-reload`、`memory`、`perf`、`ask-user`、`subagent` 与 `ui-logic` 这几个模式**不调用模型**；其余模式（含不给
