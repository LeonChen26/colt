 ...
 * **纯字符串运算**，先于任何 fs 访问——根外的绝对路径在 `isWithinRoot` 就返回，不会去碰磁盘，
...
export function readFileWithin(root: string, requestedPath: string): FileReadResult {
  ...
  if (!isWithinRoot(rootReal, absolute)) throw new Error("路径越界：只能读取项目内的文件");

  // 软链接可以把「根内的名字」指到根外，所以必须在 realpath 之后再判一次
  const targetReal = realpathSync(absolute);
  if (!isWithinRoot(rootReal, targetReal)) throw new Error("路径越界：只能读取项目内的文件");
