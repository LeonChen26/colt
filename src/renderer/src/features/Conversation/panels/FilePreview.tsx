 ...
 *
 * 预览按类型分流，**不做「尽力渲染半截内容」**：文本里 `.md` 走 Markdown，
 * 其余**按扩展名认语言**后交给 `CodeView` 着色 + 行号（认不出就原样等宽，不猜）；
...
/** 走 Markdown 渲染的扩展名；其余文本一律交给 CodeView（按扩展名着色） */
...
export function FilePreview({
  ...
  // 识别出的语言（只在文本分支有意义）。显示出来是为了「按格式渲染」这件事**可见**：
  // 颜色是主观的，语言名是可核对的。
  const language = result?.kind === "text" ? detectLanguage(path) : null;
    ...
    <div className="flex min-h-0 w-full flex-1 flex-col" data-file-view={path}>
      ...
      {loading ? (
        ...
        MARKDOWN_EXT.test(path) ? (
          ...
          <CodeView path={path} text={result.text} />
        )
      ) : result.kind === "image" ? (
