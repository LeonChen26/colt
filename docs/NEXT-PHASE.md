...
>   ```
>
>   看 `out/.smoke-mcp-e2e.png.log` 末行。免费的 33 条单测覆盖装载 / 包装 / 分页 / 远程（真实 HTTP / SSE）
...
>   ```
>
>   看 `out/.smoke-mcp-reload.png.log` 末行是否 `通过 12/12`。免费的 33 条单测覆盖 runtime 的
