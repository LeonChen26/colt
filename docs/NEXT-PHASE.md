...
3. **工作区界面端到端（改右栏 ⑦ / ⑥ 必跑）**：`dock` 模式在主进程里驱动渲染层、**真派发鼠标事件**模拟拖拽，
   ...
   **3-d. 子代理呈现（subagent）：改 ④ 的工具卡 / `MessageList.tsx` / `SubagentPreview.tsx` /
   ...
   看 `out/.smoke-subagent.png.log` 末行是否 `通过 24/24`。它从主进程推**受控视图**驱动（同
...
>   ```
>
>   看 `out/.smoke-mcp-e2e.png.log` 末行。免费的 37 条单测覆盖装载 / 包装 / 分页 / 远程（真实 HTTP / SSE）
...
>   ```
>
>   看 `out/.smoke-mcp-reload.png.log` 末行是否 `通过 12/12`。免费的 37 条单测覆盖 runtime 的
