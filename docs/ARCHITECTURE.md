...
| 目录浏览 | 单层 **500** 条（`LIST_ENTRY_LIMIT`，`main/file-list.ts`） | 「文件」页签的懒加载树每次只列一层，无界目录会把 IPC 与渲染层撑爆。截断用 `truncated` 如实说；`.git` / `node_modules` 刻意不列但返回在 `hidden` 里——UI 标注「已隐藏」而非静默吞掉；恰好 500 条不算截断（「还有下一条却装不下」才算） |
| 终端（PTY） | 每会话 **1** 个；输出缓冲 **64KB** 环形；推送 **16ms** 合帧（`main/terminal-host.ts`） | 「终端」页签背后是真 shell（ConPTY）。生命周期锚主进程——切页签不杀（渲染层卸载只取消订阅），三层收口：页签 × / 会话 dispose / app 退出；切页签期间的输出靠环形缓冲回放（`open` 幂等返回 `{replay, nextSeq}`）。16ms 合帧把逐字符回显压到每帧一次 IPC |
